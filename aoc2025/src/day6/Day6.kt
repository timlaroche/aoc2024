...
class Day6(isTest: Boolean) : Day(isTest) {
    ...
    fun part2() {
        val input = Helper2025.readAsGridForDay6_2025(inputFile)
        val rowSize = input.size
        val columnSize = input[0].size
        ...
        val parsed = problems.map { problem ->
            val operator = problem.inputs.subList(0, 1).map { it }.first()
            val inputs = rejigInputs(problem.inputs.subList(1, problem.inputs.size))
            Problem(operator, inputs.filter { it != 0L })
        }
    ...
    fun rejigInputs(inputs: List<String>): List<Long> {
        val stringLength = inputs.maxOf { it.length }
        val res = mutableListOf<Long>()
        for (i in stringLength - 1 downTo 0) {
            val sb = StringBuilder()
            inputs.forEach { it ->
                val possibleChar = it.getOrElse(i, {' '})
                if (possibleChar != '0') sb.append(possibleChar)
            }
            val finalString = sb.toString().trim()
            if (finalString.isNotEmpty()){
                res.add(finalString.reversed().toLong())
            }
        }
        return res
    }
